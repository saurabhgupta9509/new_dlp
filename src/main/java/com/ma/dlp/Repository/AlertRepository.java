...
public interface AlertRepository extends JpaRepository<Alert, Long> {
    ...
    Long countByCreatedAtBetweenAndAgentId(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("agentId") Long agentId);

    @Modifying
    @Transactional
    @Query("DELETE FROM Alert a WHERE a.createdAt BETWEEN :start AND :end AND (:agentId IS NULL OR a.agent.id = :agentId)")
    int deleteByCreatedAtBetweenAndAgentId(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end, @Param("agentId") Long agentId);
